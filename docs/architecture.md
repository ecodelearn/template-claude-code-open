...
│
├── AGENTS.md              [1a] Universal mirror — compatible with Cursor, Windsurf, Copilot / Espelho universal
...
## 1a. AGENTS.md — universal mirror / espelho universal

The `AGENTS.md` at the root is automatically generated by `project-init` as a mirror of `CLAUDE.md`.

**Why it exists / Por que existe:** tools like Cursor, Windsurf, and Copilot don't recognize `CLAUDE.md`. `AGENTS.md` is widely accepted across AI coding assistants.

**How it's maintained / Como é mantido:**
- `project-init` generates both (`CLAUDE.md` + `AGENTS.md`)
- `adopt.sh` and `adopt.ps1` copy both when adopting existing projects
- `CLAUDE.md` is the source of truth — keep them in sync if editing manually
...
**TDD Red/Green/Refactor** — when implementing any feature:
1. **Red** — write the test **before** production code (defines expected behavior)
2. **Green** — implement minimum code to pass the test
3. **Refactor** — improve without breaking tests

The cycle is mandatory for every issue, bugfix, and feature. The spec template includes a dedicated checklist in the "TDD: Red/Green" section.
...
- TDD cycle verified (Red → Green → Refactor)
